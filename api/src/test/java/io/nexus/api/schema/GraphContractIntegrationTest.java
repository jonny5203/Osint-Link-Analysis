...
class GraphContractIntegrationTest {
    ...
    private static void assertIndexDefinitions(Session session) {
        ...
        List<Record> indexes = session.run(
            ...
            """,
            Map.of("names", expectedNames)
        ).list();

        assertThat(indexes).hasSize(3);
        assertThat(indexes).allSatisfy(index ->
            assertThat(index.get("state").asString())
    ...
    private static String readResource(String resource) throws IOException {
        try (
            InputStream input =
                GraphContractIntegrationTest.class.getResourceAsStream(resource)
        ) {
            if (input == null) {
